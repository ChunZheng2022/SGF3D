...
# class MSFSKD(nn.Module):
#     def __init__(self, pseudo_in, valid_in, outplanes):
#         super(MSFSKD, self).__init__()
#         self.pseudo_in = pseudo_in
#         self.valid_in = valid_in
#         self.fc1 = nn.Linear(valid_in, valid_in)
#         self.fc2 = nn.Linear(valid_in, valid_in)
#         self.conv1 = torch.nn.Conv1d(valid_in + valid_in, outplanes, 1)
#         self.bn1 = torch.nn.BatchNorm1d(outplanes)
#
#     def forward(self, pseudo_features, valid_features):
#         # feat_learner = F.relu(self.leaners[idx](pts_feat))
#         pseudo_feas_f = pseudo_features.transpose(1,2).contiguous().view(-1, self.pseudo_in)
#         valid_feas_f = valid_features.transpose(1,2).contiguous().view(-1, self.valid_in)
#         feat_cat = torch.cat([pseudo_feas_f, valid_feas_f], dim=0)
#         feat_cat = self.fc1(feat_cat)
#         feat_weight = torch.sigmoid(self.fc2(feat_cat))
#         fusion_features = F.relu(feat_cat * feat_weight)
#
#         return fusion_features
