...
class SGFHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.point_stages = model_cfg.Point_STAGES
        self.pseudo_stages = model_cfg.Pseudo_STAGES
        ...
        att_pre_channel = GRID_SIZE * GRID_SIZE * GRID_SIZE * c_out

        '''adjustable cascade structure'''
        for i in range(self.stages):
            pre_channel = self.model_cfg.SHARED_FC[-1]*2
            cls_fc_list = []
            for k in range(0, self.model_cfg.CLS_FC.__len__()):
                cls_fc_list.extend([
                    nn.Linear(att_pre_channel, self.model_cfg.CLS_FC[k], bias=False),
                    nn.BatchNorm1d(self.model_cfg.CLS_FC[k]),
                    nn.ReLU()
                ])
                att_pre_channel = self.model_cfg.CLS_FC[k]
        self.att_shared_channel = att_pre_channel
        self.cross_attention_layers = nn.ModuleList()
        for i in range(self.stages):
            self.cross_attention_layers.append(CrossAttention(self.att_shared_channel))


        # self.fusion_fc = nn.Linear(1024 , 512)
        # self.fusion_conv = nn.Conv2d(in_channels=1024,out_channels=512,kernel_size=1,stride=1,padding=0)

        self.shared_cascade_layers = nn.ModuleList()
        for i in range(self.stages):
            ...
            self.shared_cascade_layers.append(nn.Sequential(*shared_fc_list))
            break

        '''adjustable cascade structure point'''
        for i in range(self.point_stages):
            pre_channel = self.model_cfg.SHARED_FC[-1]*2
            point_cls_fc_list = []
            for k in range(0, self.model_cfg.CLS_FC.__len__()):
                point_cls_fc_list.extend([
                    nn.Linear(att_pre_channel, self.model_cfg.CLS_FC[k], bias=False),
                    nn.BatchNorm1d(self.model_cfg.CLS_FC[k]),
                    nn.ReLU()
                ])
                point_att_pre_channel = self.model_cfg.CLS_FC[k]
        self.point_att_shared_channel = point_att_pre_channel
        self.point_cross_attention_layers = nn.ModuleList()
        for i in range(self.point_stages):
            self.point_cross_attention_layers.append(CrossAttention(self.point_att_shared_channel))

        # self.fusion_fc = nn.Linear(1024 , 512)
        # self.fusion_conv = nn.Conv2d(in_channels=1024,out_channels=512,kernel_size=1,stride=1,padding=0)

        self.point_shared_cascade_layers = nn.ModuleList()
        for i in range(self.point_stages):
            point_pre_channel = GRID_SIZE * GRID_SIZE * GRID_SIZE * c_out
            shared_fc_list = []
            for k in range(0, self.model_cfg.SHARED_FC.__len__()):
                shared_fc_list.extend([
                    nn.Linear(point_pre_channel, self.model_cfg.SHARED_FC[k], bias=False),
                    nn.BatchNorm1d(self.model_cfg.SHARED_FC[k]),
                    nn.ReLU(inplace=True)
                ])
                point_pre_channel = self.model_cfg.SHARED_FC[k]

                if k != self.model_cfg.SHARED_FC.__len__() - 1 and self.model_cfg.DP_RATIO > 0:
                    shared_fc_list.append(nn.Dropout(self.model_cfg.DP_RATIO))
            self.point_shared_cascade_layers.append(nn.Sequential(*shared_fc_list))
            break

        '''adjustable cascade structure pseudo'''
        for i in range(self.pseudo_stages):
            pre_channel = self.model_cfg.SHARED_FC[-1] * 2
            point_cls_fc_list = []
            for k in range(0, self.model_cfg.CLS_FC.__len__()):
                point_cls_fc_list.extend([
                ...
                point_att_pre_channel = self.model_cfg.CLS_FC[k]
        self.point_att_shared_channel = point_att_pre_channel
        self.pseudo_cross_attention_layers = nn.ModuleList()
        for i in range(self.pseudo_stages):
            self.pseudo_cross_attention_layers.append(CrossAttention(self.point_att_shared_channel))

        # self.fusion_fc = nn.Linear(1024 , 512)
        # self.fusion_conv = nn.Conv2d(in_channels=1024,out_channels=512,kernel_size=1,stride=1,padding=0)

        self.pseudo_shared_cascade_layers = nn.ModuleList()
        for i in range(self.pseudo_stages):
            point_pre_channel = GRID_SIZE * GRID_SIZE * GRID_SIZE * c_out
            shared_fc_list = []
            for k in range(0, self.model_cfg.SHARED_FC.__len__()):
                ...
                if k != self.model_cfg.SHARED_FC.__len__() - 1 and self.model_cfg.DP_RATIO > 0:
                    shared_fc_list.append(nn.Dropout(self.model_cfg.DP_RATIO))
            self.pseudo_shared_cascade_layers.append(nn.Sequential(*shared_fc_list))
            break
        ...
        # self.new_fusion_layer = MSFSKD(c_out, c1, self.model_cfg.ATTENTION_FC[0])
        self.conv1d = nn.Conv1d(c_out*4, 512, 1)
        ...
        if self.training:
            ...
            self.aux_self_attention_valid = CrossAttention(pre_channel_valid)
            self.aux_self_attention1_pseudo = CrossAttention(pre_channel_pseudo)
        ...
        self.build_losses_similar(self.model_cfg.LOSS_CONFIG)
    ...
    def build_losses_similar(self, losses_cfg):
        self.add_module(
            'reg_loss_func_similar',
            loss_utils.WeightedSmoothL1Loss(code_weights=losses_cfg.LOSS_WEIGHTS_SIMILAR['code_weights'])
        )
    ...
    def forward(self, batch_dict):
        ...
        glob_point = []
            ...
            # RoI Grid Pool 点云特征直接送入RoI Pooling
        x_valid = self.roi_grid_pool(batch_dict)
        x_valid  = x_valid.transpose(1,2)

        pooled_valid_features = x_valid.contiguous().view(x_valid.size(0), -1)  # 多维展平成一维
        for i in range(self.point_stages):
            stage_id = str(i)
            shared_point_features = self.point_shared_cascade_layers[0](pooled_valid_features)
            shared_point_features = shared_point_features.unsqueeze(0)  # 1,B,C
            all_point_features.append(shared_point_features)
            pre_point_feat = torch.cat(all_point_features, 0)

            # cur_point_feat = self.point_cross_attention_layers[i](pre_point_feat, shared_point_features)
            # glob_point = cur_point_feat
            glob_point=shared_point_features

            # RoI Point Pool 伪点云RoI Pooling
        ...
        points_features_expand = self.cpconvs_layer(points_features, points_neighbor)[1:] #CPConv
        ...
        pooled_pseudo_features = x_pseudo.contiguous().view(x_pseudo.size(0), -1)  # 多维展平成一维
        for i in range(self.pseudo_stages):
            shared_pseudo_features = self.pseudo_shared_cascade_layers[0](pooled_pseudo_features)
            shared_pseudo_features = shared_pseudo_features.unsqueeze(0)  # 1,B,C
            all_pseudo_features.append(shared_pseudo_features)
            pre_pseudo_feat = torch.cat(all_pseudo_features, 0)

            # cur_pesudo_feat = self.pseudo_cross_attention_layers[i](pre_pseudo_feat, shared_pseudo_features)
            # glob_pseudo=cur_pesudo_feat
            glob_pseudo=shared_pseudo_features


        # cur_fusion_features = torch.cat([glob_pseudo, glob_point], dim=2) #1 256 nC
        cur_fusion_features1 = self.cross_attention_layers[0](glob_point,glob_point)
        cur_fusion_features2 = self.cross_attention_layers[1](glob_pseudo,glob_pseudo)
        cur_fusion_features = torch.cat([cur_fusion_features1, cur_fusion_features2], dim=2) #1 256 nC
        # cur_fusion_features = cur_fusion_features1
        # cur_fusion_features = cur_fusion_features2

        '''2d conv'''
        # cur_fusion_features = cur_fusion_features.squeeze(0)
        # cur_fusion_features = cur_fusion_features.unsqueeze(-1)
        # cur_fusion_features = cur_fusion_features.unsqueeze(-1)
        # cur_fusion_features = self.fusion_conv(cur_fusion_features) #1 256 512
        # cur_fusion_features = cur_fusion_features.squeeze(-1)
        # cur_fusion_features = cur_fusion_features.squeeze(-1)
        # cur_fusion_features = cur_fusion_features.unsqueeze(0)
        # cur_fusion_features = cur_fusion_features.squeeze(0)
        # cur_fusion_features = F.relu(self.bn1(cur_fusion_features))

        '''fc'''
        # cur_fusion_features = self.fusion_fc(cur_fusion_features) #1 256 512


        '''pool'''
        cur_fusion_features = nn.functional.adaptive_max_pool1d(input=cur_fusion_features, output_size=512)
        # cur_fusion_features = glob_point
        # '''1dconv+bn'''
        # cur_fusion_features = cur_fusion_features.transpose(1,2)
        # cur_fusion_features = F.relu(self.bn1(self.conv1d(cur_fusion_features)))
        # cur_fusion_features = cur_fusion_features.transpose(1,2)
        ...
        all_fusion_features.append(cur_fusion_features)

        # fusion_features = self.fusion_layer(x_valid, x_pseudo) #55296 128
        # fusion_features = fusion_features.t()

        shared_features = cur_fusion_features
        shared_features = shared_features.squeeze(0) #256 512


        # shared_features = self.shared_fc_layer(fusion_features) #256 512
        ...
        if self.training:
            ...
            # shared_features_pseudo = shared_features_pseudo.unsqueeze(0)
            # shared_features_pseudo = self.aux_self_attention1_pseudo(shared_features_pseudo,shared_features_pseudo)
            # shared_features_pseudo = shared_features_pseudo.squeeze(0)
            ...
            # shared_features_valid = shared_features_valid.unsqueeze(0)
            # shared_features_valid = self.aux_self_attention_valid(shared_features_valid,shared_features_valid)
            # shared_features_valid = shared_features_valid.squeeze(0)
    ...
    def get_loss(self, tb_dict=None):
        ...
        #
        # rcnn_loss_similar = 0
        # rcnn_loss_cls_similar, cls_tb_dict_similar = self.get_box_cls_layer_loss_similar(self.forward_ret_dict)
        # rcnn_loss_similar += rcnn_loss_cls_similar
        # tb_dict.update(cls_tb_dict_similar)
        # rcnn_loss_reg_similar, reg_tb_dict_similar = self.get_box_reg_layer_loss_similar(self.forward_ret_dict)
        # rcnn_loss_similar += rcnn_loss_reg_similar
        # tb_dict.update(reg_tb_dict_similar)
        #
        # tb_dict['rcnn_loss_similar'] = rcnn_loss_similar.item()
        # rcnn_loss += rcnn_loss_similar


        return rcnn_loss, tb_dict

    def get_box_cls_layer_loss_similar(self, forward_ret_dict):
        loss_cfgs = self.model_cfg.LOSS_CONFIG
        rcnn_cls = forward_ret_dict['rcnn_cls_valid']
        rcnn_cls_labels = forward_ret_dict['rcnn_cls_pseudo'].view(-1)
        ...
        rcnn_loss_cls = rcnn_loss_cls * loss_cfgs.LOSS_WEIGHTS_SIMILAR['rcnn_cls_weight']
        tb_dict = {'rcnn_loss_cls_similar': rcnn_loss_cls.item()}
        return rcnn_loss_cls, tb_dict

    def get_box_reg_layer_loss_similar(self, forward_ret_dict):
        loss_cfgs = self.model_cfg.LOSS_CONFIG
        code_size = self.box_coder.code_size
        reg_valid_mask = forward_ret_dict['reg_valid_mask'].view(-1)
        gt_boxes3d_ct = forward_ret_dict['gt_of_rois'][..., 0:code_size]
        gt_of_rois_src = forward_ret_dict['gt_of_rois_src'][..., 0:code_size].view(-1, code_size)
        rcnn_reg_valid = forward_ret_dict['rcnn_reg_valid']
        rcnn_reg_pseudo = forward_ret_dict['rcnn_reg_pseudo']
        ...
        if loss_cfgs.SIMILAR_LOSS == 'smooth-l1':

            rcnn_loss_reg = self.reg_loss_func_similar(
                rcnn_reg_valid.view(rcnn_batch_size, -1).unsqueeze(dim=0),
                rcnn_reg_pseudo.view(rcnn_batch_size, -1).unsqueeze(dim=0),
            )
            rcnn_loss_reg = (rcnn_loss_reg.view(rcnn_batch_size, -1) * fg_mask.unsqueeze(dim=-1).float()).sum() / max(fg_sum, 1)
            rcnn_loss_reg = rcnn_loss_reg * loss_cfgs.LOSS_WEIGHTS_SIMILAR['rcnn_reg_weight']
            tb_dict['rcnn_loss_reg_similar'] = rcnn_loss_reg.item()
